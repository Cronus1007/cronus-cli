    ...
    static async generateSignature(argv,options) {
        ...
        return new Promise((f, r) => forge.pki.rsa.generateKeyPair(
            ...
            .then(keypair => {
                console.log('[Enc/Dec]');
                let priv, pub;
                fs.readFile(options.privinput, 'utf8', function(err, data){
                    priv =  forge.pki.privateKeyFromPem(data);
                });
                fs.readFile(options.pubinput, 'utf8', function(err, data){
                    pub = forge.pki.publicKeyFromPem(data);
                });
                return keypair;
            }).then(keypair => {
